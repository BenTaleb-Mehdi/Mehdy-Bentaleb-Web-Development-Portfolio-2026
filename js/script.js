...
function animate() {
    ...
    scene.traverse((child) => {
        if (child.isMesh && child.userData.rotationSpeed) {
            ...
            const time = Date.now() * 0.001;
            child.material.color.setHSL(0.3 + Math.sin(time + child.userData.floatOffset) * 0.1, 1, 0.5);
        }
    });
...
function updatePerformanceStats() {
    ...
    if (window.frameCount % 60 === 0) { // Update every 60 frames (~1 second)
...
function loadFeaturedProjects() {
    ...
    const featuredProjects = staticData.projects.filter(p => p.featured).sort((a, b) => a.order - b.order);
    ...
    container.innerHTML = skills.map(skill => `
        <div class="skill-item">
            <div class="skill-icon">
                ${skill.icon ? `<i class="${skill.icon}"></i>` : `<span>${skill.name.substring(0, 2)}</span>`}
            </div>
            <h3>${skill.name}</h3>
            <div class="skill-bar">
    ...
    container.innerHTML = education.map(edu => `
        <div class="education-item">
            ...
            <div class="education-content">
                ...
                <p>${edu.description}</p>
                ${edu.grade ? `<span class="education-grade">Grade: ${edu.grade}</span>` : ''}
            </div>
        </div>
    `).join('');
}
...
function loadComments() {
    ...
    const approvedComments = comments.filter(c => c.approved).sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    
    if (approvedComments.length === 0) {
        container.innerHTML = '<p class="no-comments">No comments yet. Be the first to leave a comment!</p>';
        return;
    }
