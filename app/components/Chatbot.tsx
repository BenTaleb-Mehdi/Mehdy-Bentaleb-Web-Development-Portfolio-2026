  ...
  return (
    ...
    <AnimatePresence>
        {!isOpen && (
          ...
          >
            {/* Hna derna l-Robot Icon */}
            <RobotIcon className="w-6 h-6" />
          </motion.button>
        )}
      </AnimatePresence>

      {/* Chat Window (Modern Widget) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }}
            transition={{ duration: 0.2 }}
            className="fixed bottom-6 left-6 z-50 w-[360px] h-[500px] bg-white border border-gray-100 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col"
          >
            {/* Header Pro */}
            ...
            <form onSubmit={sendMessage} className="p-3 bg-white border-t border-gray-100 shrink-0">
              <div className="relative flex items-center">
                <input
                  ...
                  // Hna fixit l-ktaba li makantch katban: text-[#111111] w bg-gray-100
